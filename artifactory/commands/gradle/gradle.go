...
func WriteInitScript(initScript string) error {
	...
	if err := os.MkdirAll(initScriptsDir, 0755); err != nil { // #nosec G703 -- path is sanitized with filepath.Clean above
	...
	if err := os.WriteFile(jfrogInitScriptPath, []byte(initScript), 0644); err != nil { // #nosec G703 -- path is sanitized with filepath.Clean
